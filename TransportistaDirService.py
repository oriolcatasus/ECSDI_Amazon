...
def register():
    ...
    def process_register():
        ...
        subject = agn[agn_uri + str(mss_cnt)]
        dsgraph.add((subject, RDF.type, FOAF.Agent))
        dsgraph.add((subject, FOAF.name, agn_name))
        dsgraph.add((subject, DSO.Address, agn_add))
        dsgraph.add((subject, DSO.AgentType, agn_type))

        # Generamos un mensaje de respuesta
        return build_message(Graph(),
            ACL.confirm,
            sender=TransportistaDirAgent.uri,
            receiver=agn_uri,
            msgcnt=mss_cnt)

    def process_unregister():
        # Si la hay extraemos la direccion del agente
        global mss_cnt

        mss_cnt += 1
        logger.info('Peticion de borrar del registro')
        agn_add = gm.value(subject=content, predicate=DSO.Address)

        subject = next(dsgraph.subjects(DSO.Address, agn_add))
        dsgraph.remove((subject, None, None))
    ...
    def process_search():
        ...
        if rsearch is not None:
            ...
            for item in rsearch:
                ...
                logger.info('Address: ' + agn_add)
                agn_name = dsgraph.value(subject=agn_uri, predicate=FOAF.name)
                logger.info('Name: ' + agn_name)
                subject = agn[agn_uri]
                gr.add((subject, RDF.type, FOAF.Agent))        
                gr.add((subject, DSO.Address, agn_add))
                gr.add((subject, DSO.Uri, agn_uri))
                gr.add((subject, DSO.Name, agn_name))
